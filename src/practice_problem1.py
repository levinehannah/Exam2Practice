...
class Box(object):
    ...
    def __init__(self, contents, volume):
        ...
        self.history = []
        self.volume = volume
        self.originvol = volume
        self.contents = contents
        if (len(contents)) > volume:
            self.contents= ''
        self.origincont = self.contents
        return None
    ...
    def append_string(self, additional_contents):
        ...
        if len(additional_contents) > space:
            numberofcharacterstoadd = space
        else:
            numberofcharacterstoadd= len(additional_contents)

        stufftoadd = ''
        for k in range(numberofcharacterstoadd):
            stufftoadd += additional_contents[k]

        self.contents += stufftoadd


        returnpo = ''
        for k in range(len(stufftoadd),len(additional_contents)):
            returnpo += additional_contents[k]
        return returnpo
    ...
    def double_then_shrink(self, new_volume):
        ...
        # --------------------------------------------------------------
        numdis = 0
        numdis += len(self.double()) + len(self.shrink(new_volume))
        return numdis
    ...
    def reset(self):
        ...
        #    TIME ESTIMATE:   5 minutes.
        # --------------------------------------------------------------
        self.history.extend([self.contents])
        self.volume = self.originvol
        self.contents = self.origincont
    ...
    def steal(self, other_box):
        ...
        self.contents += other_box.contents.replace(Box.append_string(self, other_box.contents), '')
    ...
    def combined_box(self, other_box):
        ...
        newsh = self.contents + other_box.contents
        othernewsh = self.volume + other_box.volume
        newbox = Box(newsh, othernewsh)
